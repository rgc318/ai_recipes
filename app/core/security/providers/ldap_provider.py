...
class LDAPProvider(CredentialsProvider):
    ...
    def search_user(self, conn: LDAPObject) -> list[tuple[str, dict[str, list[bytes]]]] | None:
        ...
        If none or multiple user are found, return False
        """
        if not self.data:
            return None
        ...
        if len(user_entry) > 1:
            self._logger.warning("[LDAP] Multiple user found with the provided user filter")
    ...
    def get_user(self) -> PrivateUser | None:
        ...
        if user is None:
            self._logger.debug("[LDAP] User is not in app. Creating a new account")
